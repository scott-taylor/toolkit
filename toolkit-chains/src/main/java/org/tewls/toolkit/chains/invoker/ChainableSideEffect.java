 ...
 * @param <I>
 */
final class ChainableSideEffect<S, I> implements Chainable<S, I>
{
    private final Chainable<S, I> previous;
    private final SideEffect sideEffect;

    public ChainableSideEffect(Chainable<S, I> c, SideEffect s)
    {
        previous = c;
        sideEffect = s;
    }
    ...
    @Override
    public Invoker<S> link(final Invoker<? super I> invoker)
    {
        return previous.link(new Invoker<I>()
        {
            ...
            {
                sideEffect.process(new BoundedInvokerAdaptor<I>(invoker, input));
            }
        });
    }
